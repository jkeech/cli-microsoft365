...
describe(commands.TENANT_REPORT_SERVICESUSERCOUNTS, () => {
  ...
  it('has correct name', () => {
    assert.equal(command.name.startsWith(commands.TENANT_REPORT_SERVICESUSERCOUNTS), true);
  });
  ...
  it('retrieves the report by type for the given period', (done) => {
    ...
    cmdInstance.action({ options: { debug: false, period: 'D7' } }, () => {
      try {
        assert.equal(requestStub.lastCall.args[0].url, "https://graph.microsoft.com/v1.0/reports/getOffice365ServicesUserCounts(period='D7')");
  ...
  it('has help referring to the right command', () => {
    ...
    cmd.help({}, () => { });
    assert(find.calledWith(commands.TENANT_REPORT_SERVICESUSERCOUNTS));
  });
