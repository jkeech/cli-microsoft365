...
const command: Command = require('./report-mailboxusagestorage');
...
describe(commands.OUTLOOK_REPORT_MAILBOXUSAGESTORAGE, () => {
  ...
  it('gets the report for the last week', (done) => {
    ...
    cmdInstance.action({ options: { debug: false, period: 'D7' } }, () => {
      try {
        assert.equal(requestStub.lastCall.args[0].url, "https://graph.microsoft.com/v1.0/reports/getMailboxUsageStorage(period='D7')");
