...
describe(commands.OUTLOOK_REPORT_MAILAPPUSAGEUSERCOUNTS, () => {
  ...
  it('has correct name', () => {
    assert.equal(command.name.startsWith(commands.OUTLOOK_REPORT_MAILAPPUSAGEUSERCOUNTS), true);
  });
  ...
  it('gets the report for the last week', (done) => {
    ...
    cmdInstance.action({ options: { debug: false, period: 'D7' } }, () => {
      try {
        assert.equal(requestStub.lastCall.args[0].url, "https://graph.microsoft.com/v1.0/reports/getEmailAppUsageUserCounts(period='D7')");
  ...
  it('has help referring to the right command', () => {
    ...
    cmd.help({}, () => { });
    assert(find.calledWith(commands.OUTLOOK_REPORT_MAILAPPUSAGEUSERCOUNTS));
  });
});