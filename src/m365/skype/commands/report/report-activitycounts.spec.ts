...
describe(commands.SKYPE_REPORT_ACTIVITYCOUNTS, () => {
  ...
  it('has correct name', () => {
    assert.equal(command.name.startsWith(commands.SKYPE_REPORT_ACTIVITYCOUNTS), true);
  });
  ...
  it('gets the report for the last week', (done) => {
    ...
    cmdInstance.action({ options: { debug: false, period: 'D7' } }, () => {
      try {
        assert.equal(requestStub.lastCall.args[0].url, "https://graph.microsoft.com/v1.0/reports/getSkypeForBusinessActivityCounts(period='D7')");
  ...
  it('has help referring to the right command', () => {
    ...
    cmd.help({}, () => { });
    assert(find.calledWith(commands.SKYPE_REPORT_ACTIVITYCOUNTS));
  });
});